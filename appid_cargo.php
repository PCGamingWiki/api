...
$config = array(
	...
	'api_pcgw' => 'https://www.pcgamingwiki.com/w/api.php?action=cargoquery&tables=Infobox_game&fields=_pageName%3DPage&where=Steam_AppID%20HOLDS%20%27%APPID%%27&format=json',
...
else
{
	...
	$results = $json['cargoquery'];

	// Get first results url
	$first_result = reset( $results );
	$article_url = urlencode( $first_result['title']['Page'] );
}

if ( count( $results ) == 1 )
{
	// Redirect
	header( 'Location: ' . 'https://www.pcgamingwiki.com/wiki/' . $article_url );
	die();
}
?>
<!DOCTYPE html>
		...
		<div class="content">
			...
			if ( count( $results ) == 0 )
			{
				...
				if ( count( $results ) != 0 )
				{
					...
					print '<a href="//pcgamingwiki.com/w/index.php?title=' . $results['name'] . '&amp;redlink=1&amp;action=edit"><div class="create-page-button">Create Page</div></a>';
				}
				else
				{
					print "No such AppID.";
			...
			else if ( count( $results ) > 1 )
			{
				...
				{
					print "<p><a href='https://www.pcgamingwiki.com/wiki/'" . urlencode( current( $results )['title']['Page'] ) . ">" . current( $results )['title']['Page'] . "</a></p>";
					next( $results );
				}
			}
			?>
